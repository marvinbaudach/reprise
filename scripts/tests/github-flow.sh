...
require_pattern '(^|[[:space:]])meson([[:space:]]|$)' "$workflow"
require_pattern '^      CARGO_INCREMENTAL: 0$' "$workflow"
require_pattern '^      CARGO_PROFILE_DEV_DEBUG: 0$' "$workflow"
require_pattern '^      CARGO_PROFILE_TEST_DEBUG: 0$' "$workflow"
require_pattern '^      DISPLAY_TEST_JOBS: 1$' "$workflow"
