...
test('the Hero seek strip presents the measured duration in spectral-fill mode', async () => {
  ...
  assert.ok(hero);
  assert.match(hero, /data-showcase="hero-seek-track"[^>]+data-seek-mode="fill"/);
  assert.match(hero, /src="\/reprise\/brand\/reprise-mark\.svg"[^>]+width="26" height="26"/);
  assert.match(hero, />0:00</);
  assert.match(hero, /data-seek-canvas="" data-seek-mode="fill"/);
