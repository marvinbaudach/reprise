...
- **SRC-12** [active] [gtk] — Episodes can be selected in bulk in both the
  ...
  itself with a single aggregated toast and a single undo. Escape clears the
  current episode selection in whichever of the two surfaces is showing, and
  is passed on untouched when nothing is selected.
