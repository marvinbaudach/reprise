...
Performance evidence: the retained isolated release run at `/tmp/reprise-glass-underlay.rcQbCn` measured the unchanged AlbumView glow at 5.428 ms cold downscale, 0.148 ms cold blur, 5.576 ms total, 0.795 microseconds p95 over 1,000 cache hits, and a 4 KiB decoded texture. The 10,000-row baseline seed completed, but the paired 120-frame baseline/glass process stopped before app launch because this managed sandbox rejected the private D-Bus socket with `Operation not permitted`; no frame-time or GPU-completion number is claimed. The new adapter adds one wrapper, one inner adjustment, and constant-time value projection per native scroller while retaining recycled GTK rows/cells; it adds no per-album or per-track widget cost.
