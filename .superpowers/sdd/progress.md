...
Performance evidence: the retained isolated release run at `/tmp/reprise-glass-underlay.rcQbCn` measured the unchanged AlbumView glow at 5.428 ms cold downscale, 0.148 ms cold blur, 5.576 ms total, 0.795 microseconds p95 over 1,000 cache hits, and a 4 KiB decoded texture. The first host attempt at `/tmp/reprise-glass-results.N9tY74/run` measured 3.462 ms downscale, 0.119 ms blur, 3.581 ms total, and 0.556 microseconds p95, then exposed the now-fixed measurement-window and cleanup defects before a baseline frame report could be written. The managed-sandbox attempt still stops before app launch because private D-Bus socket creation is denied. No paired frame-time or GPU-completion number is claimed yet. The new adapter adds one wrapper, one inner adjustment, and constant-time value projection per native scroller while retaining recycled GTK rows/cells; it adds no per-album or per-track widget cost.
