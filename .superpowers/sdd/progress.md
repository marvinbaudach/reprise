...
CUA search regression: code complete (commits 717de1b5 and c1253808, base a9ed4d6c, explicit Escape clearing now invalidates the SEARCH-6 preserved query and empty-query collapse defers active-content focus until after GTK finishes the Escape key dispatch). Three captured host runs reproduced SEARCH-4 red: the first exposed the stale restored query, while tagged diagnostic logs from the third proved that collapse reached `search_mode=false` and synchronous `focus_active_content=true` before GTK retained `GtkText / Search all fields`. Both defects now have focused regressions; the test-isolation failure found by the first full run was reproduced and replaced with an isolated scheduler test before the final green run. 982 Core, 805 GNOME, and 73 Linux platform tests passed with strict Workspace-Clippy, warning-denied Rustdoc, architecture, UX traceability with 148 active rules, accessibility semantics, input parity, motion tokens, formatting, file size, and audit with only accepted `RUSTSEC-2024-0436`. `deferred host check`: build commit c1253808 and rerun `CUA_E2E_ONLY=populated-library scripts/cua-e2e/run.sh` outside the managed sandbox; no green post-fix CUA result is claimed yet.
