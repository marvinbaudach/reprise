...
//! `reprise://releases`. Tools: `music_search_tracks` (read),
...
const SERVER_INSTRUCTIONS: &str = "Reprise local music library and player. \
    Read-only tools and resources expose path-free track, artist, album, \
    playlist, concert, and release metadata. Playlist creation and safe \
    rename/append operations use \
