...
| `music_get_device_sync_state` | none | Read playlists and verified sync times, transfer profile, deduplicated totals, device capacity/access, delta, progress, current title, bytes/second, available controls, and the single playlists target's folder, activation, size on the device and diff reading plus its balance | read-only |
...
`music_get_device_sync_state`'s `target` object describes the one sync target
there is — playlists: its device folder, whether this device's slot for it is
active, its size on the device, and a `reading` that is always `diff`. There
is no per-target size cap. The diff carries the copy/remove file counts and
the moved/freed byte totals. `balance` restates that same diff; whether it
has anything pending is decided by file counts, never by bytes, so a
removal-only sync that moves nothing still reports pending work.
