...
#[test]
fn ac_7_audio_filter_contains_a_disabled_bounded_spectrum_analyzer() {
    gst::init().unwrap();
...
#[test]
fn ac_7_spectrum_messages_project_exactly_one_bounded_frame() {
    gst::init().unwrap();
...
#[test]
fn ac_7_enabled_player_emits_live_spectrum_frames() {
    let _guard = AUDIO_SINK_TEST_LOCK
        .lock()
        .unwrap_or_else(std::sync::PoisonError::into_inner);
    std::env::set_var(AUDIO_SINK_ENV_VAR, "fakesink");
