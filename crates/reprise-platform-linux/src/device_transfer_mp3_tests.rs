...
use super::{
    finish_output, probe_mp3_transcode_capability, transcode_to_mp3, Mp3Metadata,
    Mp3TranscodeRequest, TranscodeError, REQUIRED_MP3_FACTORIES,
};
...
fn real_audio_fixture_transcodes_to_mp3_with_required_tags_and_embedded_cover() {
    ...
    let metadata = Mp3Metadata::for_track(&track);
    assert_eq!(metadata.cover.as_deref(), Some(TINY_PNG));
    let request = Mp3TranscodeRequest {
        source: track.source_path,
        output: output.clone(),
        quality: Mp3Quality::Kbps192,
        metadata,
    };

    let encoded = transcode_to_mp3(&request, &AtomicBool::new(false)).unwrap();
...
#[test]
fn pre_cancelled_mp3_transcode_leaves_no_output() {
    let directory = tempfile::tempdir().unwrap();
    let track = tagged_audio_fixture(directory.path());
    let output = directory.path().join("cancelled.mp3");
    let request = Mp3TranscodeRequest {
        metadata: Mp3Metadata::for_track(&track),
        source: track.source_path,
        output: output.clone(),
    ...
    assert!(matches!(
        transcode_to_mp3(&request, &AtomicBool::new(true)),
