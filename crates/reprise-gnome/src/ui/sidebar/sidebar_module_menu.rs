...
mod tests {
    ...
    #[test]
    #[ignore = "requires a display; run via xvfb-run"]
    fn nav_16_a_turned_off_module_is_not_a_restorable_session_source() {
        let _main_context = crate::ui::test_main_context::lock_main_context();
        gtk4::init().unwrap();
        let conn = Rc::new(crate::test_db::open().unwrap());
        reprise_core::online_sources::set_enabled(&conn, true).unwrap();
        ...
        sidebar.refresh("test optional module disabled");
        ...
        reprise_core::modules::set_enabled(&conn, &PODCASTS_MODULE, true).unwrap();
        sidebar.refresh("test optional module restored");
        assert!(find_row(&sidebar.shared, &ViewSource::Podcasts).is_some());
    }
}
