    ...
    gtk4::glib::idle_add_local_once(move || {
        ...
        if rows > 0 && adjustment.upper() > 0.0 {
            ...
            if focus {
                let grid = grid.clone();
                gtk4::glib::idle_add_local_once(move || {
                    // `ListScrollFlags::FOCUS` only moves focus within a grid
                    // that already belongs to the window's focus chain. GRID-5
                    // and GRID-6 both arrive from another focused surface, so
                    // transfer focus to the visible grid before selecting the
                    // realized target in this second phase.
                    if !grid.state_flags().contains(gtk4::StateFlags::FOCUS_WITHIN) {
                        grid.grab_focus();
                    }
