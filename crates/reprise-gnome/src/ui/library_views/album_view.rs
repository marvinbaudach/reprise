...
use crate::ui::album_card::{
    self, AlbumCardShared, AlbumOwnedCallback, CallbackSlot, StringCallback,
};
...
pub(in crate::ui) struct AlbumView {
    ...
    on_activate: CallbackSlot<AlbumOwnedCallback>,
    on_artist_activate: CallbackSlot<StringCallback>,
}
...
impl AlbumView {
    pub(in crate::ui) fn new(conn: Rc<RefCell<Connection>>, cover_loader: Rc<CoverLoader>) -> Self {
        ...
        let on_activate: CallbackSlot<AlbumOwnedCallback> = Rc::new(RefCell::new(None));
        let on_artist_activate: CallbackSlot<StringCallback> = Rc::new(RefCell::new(None));
