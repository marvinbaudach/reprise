...
pub(in crate::ui) fn build(conn: &Rc<Db>, enabled: bool) -> RadioPreferenceRows {
    let config = reprise_core::radio::config::load(conn).unwrap_or_default();
        ...
        .build();
    {
        ...
        order.connect_selected_notify(move |row| {
            if let Err(error) = save_search_order(&conn, order_for_index(row.selected())) {
        ...
        .build();
    {
        ...
        report_plays.connect_active_notify(move |row| {
            if let Err(error) = save_report_plays(&conn, row.is_active()) {
                tracing::warn!(%error, "could not save radio preference");
            }
        });
    }
...
fn save_search_order(conn: &Db, value: SearchOrder) -> Result<(), rusqlite::Error> {
    reprise_core::radio::config::set_search_order(conn, value)
}

fn save_report_plays(conn: &Db, value: bool) -> Result<(), rusqlite::Error> {
