//! Custom header drag-reorder for any editable `ColumnView`.
//!
//! ## Why this exists
//!
...
pub(in crate::ui) fn css() -> String {
    use crate::ui::style::tokens::DROP_INDICATOR_THICKNESS;
    format!(
...
struct DragState {
    ...
    /// Erased persisted id resolved from the table model at press time.
    dragged_id: String,
...
fn marker_target(
    ...
    resolved?;
    let (target, class) = match slot {
        InsertionSlot::Before(index) => {
    ...
    if target.column == *dragged_column {
        return None;
    }
    Some((target.widget.clone(), class))
}
...
fn perform_drop(
    ...
    let remaining: Vec<&HeaderTitle> = titles
        .iter()
        .enumerate()
        .filter(|(index, _)| *index != dragged_index)
        .map(|(_, title)| title)
        .collect();
...
#[cfg(test)]
#[path = "../track_list/column_header_dnd_tests.rs"]
mod tests;
