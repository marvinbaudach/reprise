...
enum RemoteToggleAction {
    Disable,
    Enable,
    Confirm,
}

const fn remote_toggle_action(
...
pub(in crate::ui) fn remote_suggestions_row_for(
    ...
    row.connect_active_notify(move |row| {
        ...
        match remote_toggle_action(row.is_active(), preference) {
            RemoteToggleAction::Disable => {
                let result = reprise_core::library_doctor::disable_remote_suggestions(&conn);
            ...
            RemoteToggleAction::Enable => {
                let result = reprise_core::library_doctor::accept_remote_suggestions(&conn);
                if let Err(error) = result {
...
fn present_remote_confirmation(
    ...
    dialog.choose(Some(parent), gio::Cancellable::NONE, move |response| {
        ...
        match reprise_core::library_doctor::accept_remote_suggestions(&conn) {
            Ok(()) => {
