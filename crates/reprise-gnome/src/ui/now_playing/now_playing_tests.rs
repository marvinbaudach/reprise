...
#[test]
fn ac_21_visual_is_the_third_panel_tab() {
...
#[test]
#[ignore = "requires a display; run via xvfb-run"]
fn ac_21_icons_only_switcher_keeps_three_labeled_keyboard_targets() {
    gtk4::init().unwrap();
    let content = gtk4::Box::new(gtk4::Orientation::Vertical, 0);
    let widgets = test_widgets(&content, true);
    widgets
...
#[test]
#[ignore = "requires a display; run via xvfb-run"]
fn ac_21_visual_page_shrinks_instead_of_overlapping_the_tab_switcher() {
    gtk4::init().unwrap();
    let content = gtk4::Box::new(gtk4::Orientation::Vertical, 0);
    let widgets = test_widgets(&content, true);
