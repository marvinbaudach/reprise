//! SEARCH-8 / FIL-1d: the Radio section's query surface.
//!
...
impl RadioView {
    /// SEARCH-8: applies this section's query (FIL-1d: station names).
    pub(in crate::ui) fn set_search_query(&self, query: &str) {
        self.shared.filter_bar.set_query(query);
    }

    /// SEARCH-8: the bar removed the query itself, so the header entry has to
    /// follow.
