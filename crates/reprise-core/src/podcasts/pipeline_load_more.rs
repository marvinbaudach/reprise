...
pub fn load_more_youtube(
    ...
    let conn = db.conn();
    let subscription = store::subscription_in(conn, subscription_id)?
        .filter(|subscription| {
    ...
    if !crate::podcasts::config::source_network_allowed_in(conn, PodcastKind::Youtube)? {
    ...
    let baseline = store::future_only_baseline_in(conn, subscription.id)?
        .into_iter()
    ...
    for episode in &feed.episodes {
        ...
        if store::upsert_episode_in(conn, subscription.id, episode, now)?.is_some() {
            changed += 1;
        }
    }
    Ok(changed)
