...
pub const VOLUME_KEY_TRACK_SWITCH_ENABLED_KEY: &str = "playback.volume_key_track_switch_enabled";
...
/// Whether holding volume keys skips tracks while playback owns remote volume.
/// Default `true`; Android is currently the only surface that exposes it.
fn get_volume_key_track_switch_enabled_in(conn: &Connection) -> bool {
    get_bool_in(conn, VOLUME_KEY_TRACK_SWITCH_ENABLED_KEY, true).unwrap_or(true)
}

fn set_volume_key_track_switch_enabled_in(
    conn: &Connection,
    enabled: bool,
) -> Result<(), rusqlite::Error> {
    set_bool_in(conn, VOLUME_KEY_TRACK_SWITCH_ENABLED_KEY, enabled)
}
