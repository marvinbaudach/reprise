...
fn doc_apply_writes_only_checked() {
    ...
    let ids = seed(&conn, std::slice::from_ref(&path));
    let plan = plan_for(&conn, &ids, |row| row.field == DoctorField::Artist);

    let report = LibraryDoctor::new(&conn)
...
fn doc_undo_restores_previous_values() {
    ...
    let conn = crate::db::Db::open_in_memory().unwrap();
    let ids = seed(&conn, std::slice::from_ref(&path));
    let plan = plan_for(&conn, &ids, |_| true);
    let apply = LibraryDoctor::new(&conn)
        .apply_review_plan(&plan, continue_job)
        .unwrap();
    assert_eq!(read_editable_tags(&path).unwrap().artist, "Artist");

    let revert = LibraryDoctor::new(&conn)
...
fn partial_revert_keeps_pointer_and_full_revert_reveals_previous_cleanup() {
    ...
    let first_plan = plan_for(&conn, &ids[..1], |_| true);
    let first_apply = LibraryDoctor::new(&conn)
        .apply_review_plan(&first_plan, continue_job)
        .unwrap();
    let second_plan = plan_for(&conn, &ids[1..], |_| true);
    let second_apply = LibraryDoctor::new(&conn)
    ...
    .unwrap();
    let partial = LibraryDoctor::new(&conn)
        .revert_last_cleanup(continue_job)
        .unwrap()
        .unwrap();
    ...
    .unwrap();
    let complete = LibraryDoctor::new(&conn)
        .revert_last_cleanup(continue_job)
        .unwrap()
        .unwrap();
...
fn doc_5a_removed_track_is_unavailable_without_a_file_write() {
    ...
    let plan = plan_for(&conn, &ids, |_| true);
    conn.conn()
        .execute("UPDATE tracks SET removed_at=1 WHERE id=?1", [ids[0]])
        .unwrap();
...
fn doctor_apply_crash_finalization_is_db_only_and_preserves_revert_reachability() {
    ...
    assert!(LibraryDoctor::new(&conn)
        .apply_review_plan(&plan, continue_job)
        .is_err());
    assert_eq!(read_editable_tags(&path).unwrap().artist, "Artist");
    conn.conn()
        .execute_batch("DROP TRIGGER reject_doctor_apply_status;")
        .unwrap();
    drop(conn);
    ...
    assert!(LibraryDoctor::new(&reopened)
        .last_cleanup()
        .unwrap()
        .is_some());
    LibraryDoctor::new(&reopened)
...
fn doctor_revert_crash_finalization_consumes_source_without_rewriting_file() {
    ...
    assert_eq!(read_editable_tags(&path).unwrap().artist, " Artist ");
    conn.conn()
        .execute_batch("DROP TRIGGER reject_doctor_revert_status;")
        .unwrap();
    drop(conn);
    let before_finalize = std::fs::read(&path).unwrap();
...
fn doc_5a_unreadable_file_is_failed_not_unavailable() {
    ...
    let conn = crate::db::Db::open_migrated(None).unwrap();
    let ids = seed(&conn, std::slice::from_ref(&path));
    let plan = plan_for(&conn, &ids, |row| row.field == DoctorField::Artist);
