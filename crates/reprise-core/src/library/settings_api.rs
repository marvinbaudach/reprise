...
use super::{
    ...
    get_seek_colouring_in, get_seek_legend_seen_in, get_setting_in, get_sidebar_collapsed_in,
    ...
    set_info_panel_visible_in, set_last_viewed_import_errors_in, set_last_viewed_missing_in,
    set_library_root_in, set_missing_auto_clean_in, set_new_releases_fetch_completed_in,
...
pub fn get_online_discovery_banner_completed(db: &Db) -> Result<bool, rusqlite::Error> {
    let conn = db.conn();
    get_online_discovery_banner_completed_in(conn)
}

pub fn set_online_discovery_banner_completed(
    db: &Db,
    completed: bool,
) -> Result<(), rusqlite::Error> {
    let conn = db.conn();
    set_online_discovery_banner_completed_in(conn, completed)
}
...
pub fn get_seek_colouring(db: &Db) -> SeekColouring {
    let conn = db.conn();
    get_seek_colouring_in(conn)
}

pub fn set_seek_colouring(db: &Db, value: SeekColouring) -> Result<(), rusqlite::Error> {
    let conn = db.conn();
    set_seek_colouring_in(conn, value)
}

pub fn get_seek_legend_seen(db: &Db) -> u32 {
    let conn = db.conn();
    get_seek_legend_seen_in(conn)
}

pub fn set_seek_legend_seen(db: &Db, count: u32) -> Result<(), rusqlite::Error> {
    let conn = db.conn();
    set_seek_legend_seen_in(conn, count)
}
