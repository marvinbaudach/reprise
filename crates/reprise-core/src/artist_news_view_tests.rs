...
#[test]
fn nr_31_default_window_hides_releases_older_than_five_years() {
    let rows = vec![
        entry(
            "recent",
            "Recent",
...
#[test]
fn nr_31_singles_are_absent_until_their_chip_is_on() {
    let rows = vec![entry(
        "single",
        "Single",
        "Single",
...
#[test]
fn nr_31_undated_release_survives_every_window() {
    let rows = vec![entry(
        "undated",
        "Undated",
        "Album",
...
#[test]
fn nr_31_window_all_shows_the_full_catalog() {
    let filter = ReleasesFilter {
...
#[test]
fn nr_31_empty_type_selection_shows_every_type() {
    let filter = ReleasesFilter {
...
#[test]
fn nr_31_all_selected_types_with_all_window_is_the_widest_scope() {
    let filter = ReleasesFilter {
...
#[test]
fn nr_31_count_line_never_exceeds_its_total() {
    let db = crate::db::Db::open_in_memory().unwrap();
    db.conn()
        .execute(
            "INSERT INTO tracks (path, title, artist, album_artist, album, added_at)
...
/// NR-31: the default view is the quiet one, and the filter row decides that
...
#[test]
fn nr_31_a_fresh_library_loads_exactly_the_default_filter() {
    let db = crate::db::Db::open_in_memory().unwrap();
