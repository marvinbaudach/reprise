...
/// Zero (not a faint idle shimmer): the membrane is driven by the
/// current bands every tick, so anything above [`super::membrane::Membrane`]'s
...
impl VisualEngine {
    ...
    /// Track switched: reset the clock, membrane and impact overlay so
    /// leftover vibrations/sparks from the previous track don't bleed in.
