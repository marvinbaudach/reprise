//! `MTP-45` selection-candidate query for podcasts/YouTube — split out of
...
/// `MTP-45`: every episode candidate `selection::select_episodes` needs to
...
mod tests {
    ...
    /// Runs the RSS podcast rule (`MTP-45`'s `UnplayedDownloadsOnly`) over
    ...
    // `MTP-45` end-to-end: `query_selection_candidates_for_device` feeding
    ...
    #[test]
    fn mtp_45_an_unplayed_downloaded_episode_is_ready_to_copy() {
        let conn = migrated();
        let downloads = tempfile::tempdir().unwrap();
    ...
    #[test]
    fn mtp_45_selection_candidates_stay_scoped_to_shows_enabled_for_the_device() {
        let conn = migrated();
        let downloads = tempfile::tempdir().unwrap();
