...
mod tests {
    use super::*;
    use crate::device_sync::{compute_delta, SyncCandidate};

    #[test]
    fn matching_adopted_ledger_and_scan_votes_are_resident_and_plan_no_transfer() {
        let adopted = "Emmure/Speaker Of The Dead/13 song.flac";
        let inventory = vec![DeviceFileRecord {
            device_serial: "phone".into(),
            track_id: 13,
            source_path: "/music/13.flac".into(),
            source_size: 5,
            source_mtime: 7,
            device_path: adopted.into(),
            device_size: 5,
            profile_fingerprint: "copy-original-v1".into(),
            pinned: false,
        }];
        let managed_files = vec![ManagedDeviceFile {
            relative_path: adopted.into(),
            size_bytes: 5,
        }];

        assert_eq!(
            build_directory_spellings(&inventory, &managed_files)
                .get(&fold_path("Emmure/Speaker Of The Dead")),
            Some(&DirectorySpelling::Resident(
                "Emmure/Speaker Of The Dead".into()
            ))
        );
        ...
        let delta = compute_delta(&[candidate], &inventory, true);
