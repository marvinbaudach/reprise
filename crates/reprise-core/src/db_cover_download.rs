...
pub(crate) fn migrate_v84(conn: &Connection) -> Result<(), rusqlite::Error> {
    let version: i64 = conn.query_row("PRAGMA user_version", [], |row| row.get(0))?;
    if version >= 84 {
        return Ok(());
    }
...
mod tests {
    ...
    #[test]
    fn v84_reopens_only_the_cover_startup_task() {
        let conn = crate::db::open(None).unwrap();
        crate::db::migrate_connection(&conn).unwrap();
        conn.execute(
            "INSERT INTO settings (key, value) VALUES (?1, 'cover'), (?2, 'spectrogram')",
        ...
        .unwrap();
        conn.pragma_update(None, "user_version", 83).unwrap();

        migrate_v84(&conn).unwrap();
    ...
    #[test]
    fn v84_does_not_repeat_after_the_schema_version_is_current() {
        let conn = crate::db::open(None).unwrap();
        crate::db::migrate_connection(&conn).unwrap();
        conn.execute(
            "INSERT INTO settings (key, value) VALUES ('startup_tasks.completed.covers', 'new')",
