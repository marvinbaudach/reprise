...
//! `db_*.rs` file with a `migrate_v29` entry point, exactly like every
//! post-v18 migration (see `db_change_log::migrate_v28`).
...
/// Applies schema v29 — the AI-jobs/provenance/role migration — following the
...
/// share a transaction).
pub(crate) fn migrate_v29(conn: &Connection) -> Result<(), rusqlite::Error> {
