...
use crate::queue_snapshot_file::{QueueSnapshotFile, FILE_NAME};
...
fn a_snapshot_waiting_for_the_writer_commits_after_release() {
    ...
    let persister = QueuePersister::spawn(&database_path, Arc::clone(&writer)).unwrap();
...
fn a_newer_snapshot_survives_an_older_drain_removal() {
    ...
    let file = QueueSnapshotFile::new(&database_path).unwrap();
    file.write(1, &queue(vec![1], 0)).unwrap();
    file.write(2, &queue(vec![2], 0)).unwrap();

    file.remove_if_sequence(1);

    let (sequence, restored) = file.read().unwrap();
    assert_eq!(sequence, 2);
    assert_eq!(restored.snapshot(), queue(vec![2], 0).snapshot());
}
...
fn a_fresh_session_prefers_the_durable_snapshot_over_the_database() {
    ...
    let persister = QueuePersister::spawn(&library.database_path, library.writer_handle()).unwrap();
