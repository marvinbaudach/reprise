...
fn require_playlist(db: &Db, id: i64) -> Result<PlaylistSummary, CliError> {
    playlists::get(db, id)?.ok_or_else(|| CliError::NotFound(format!("playlist {id}")))
}

/// Lists every manual playlist with its track count.
pub fn list(db: &Db, json_output: bool) -> Result<(), CliError> {
    let playlists = playlists::list(db)?;
    if json_output {
...
pub fn show(db: &Db, id: i64, json_output: bool) -> Result<(), CliError> {
    let summary = require_playlist(db, id)?;
    let tracks = all_playlist_tracks(db, id)?;
    if json_output {
...
pub fn create(db: &Db, name: &str, tracks: &[i64], json_output: bool) -> Result<(), CliError> {
    let id = if tracks.is_empty() {
        retry_write(|| playlists::create(db, name))?
    } else {
        retry_write(|| playlists::create_with_tracks(db, name, tracks))?
    };
    if json_output {
...
pub fn rename(db: &Db, id: i64, name: &str, json_output: bool) -> Result<(), CliError> {
    let renamed = retry_write(|| playlists::rename(db, id, name))?;
    if renamed == 0 {
...
pub fn delete(db: &Db, id: i64, yes: bool, json_output: bool) -> Result<(), CliError> {
    if !yes {
    ...
    let deleted = retry_write(|| playlists::delete(db, id, &name))?;
    if !deleted {
        return Err(CliError::NotFound(format!("playlist {id}")));
    }
    if json_output {
